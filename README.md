...
Модель: по умолчанию **авто** — приложение перебирает цепочку моделей Gemini
(сначала дешёвые `-lite`), и если одна упёрлась в бесплатную квоту или не
ответила, запрос уходит следующей. Сбоившие модели запоминаются на час и
опускаются в конец очереди. В настройках ⚙️ можно закрепить конкретную модель —
она будет пробоваться первой, остальные останутся запасными.
