...
- **`app.js`** — all logic. Calls Gemini directly from the browser via `fetch`, walking the `FALLBACK_MODELS` chain: each model has its own free-tier daily quota, so on 429/503/404/timeout the request moves to the next one (capped at `MAX_MODEL_ATTEMPTS`). Recent failures are counted in `localStorage` (`gemini_model_fails`) and reorder the chain so healthy models go first — transient failures are forgotten after 1h, "hard" ones (404/400: the model isn't on this key) after 24h so a dead model isn't re-probed on every quiet hour. A 403 (or a 400 mentioning the API key) aborts the whole chain since no other model will help. Requests send `thinkingConfig.thinkingBudget: 0` — a dictionary entry needs no reasoning and thinking is the main latency source; a model that rejects the field (400 mentioning "thinking") is retried once without it and remembered in `gemini_no_thinking_cfg`. The model picker in settings only chooses which model is tried *first* — `auto` uses the chain order as is. Translation reliability comes from `generationConfig.responseSchema` (`RESPONSE_SCHEMA`) forcing structured JSON output, so the response is `JSON.parse`d, never regex-scraped. `SYSTEM_PROMPT` defines dictionary behavior (auto language detection, per–part-of-speech field rules). Rendering branches on `partOfSpeech` (noun → article + singular/plural, verb → infinitive, adjective → translation only).
